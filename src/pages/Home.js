        ...
        <SimpleGrid columns={{ sm: 1, md: 2, lg: 3 }} spacing={10}>
          {popularDishes.map(dish => (
            <Box key={dish.id} bg="white" boxShadow="md" borderRadius="md" overflow="hidden">
              <Image src={dish.imageUrl} alt={dish.name} width="100%" height="250px" objectFit="cover" />
              <Box p={4}>
        ...
        <SimpleGrid columns={{ sm: 1, md: 2, lg: 3 }} spacing={10}>
          {placeImages.map((image, index) => (
            <Box
              ...
              overflow="hidden"
            >
              <Image src={image} alt={`Place image ${index + 1}`} width="100%" height="250px" objectFit="cover" />
            </Box>
          ))}
        </SimpleGrid>
      </Box>
